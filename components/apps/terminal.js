...
import { AIEngine } from './ai-knowledge';
...
export class Terminal extends Component {
    constructor() {
        ...
        this.child_directories = {
            ...
            skills: ["Front-end development", "React.js", "jQuery", "Flutter", "Express.js", "SQL", "Firebase", "Backend Development", "Node.js", "Express.js", "REST APIs", "MongoDB", "SQL", "Firebase", "AI Agents", "Prompt Engineering", "Machine Learning Basics", "System Design Fundamentals"],
            projects: ["InviteCraft", "HealthAI", "DevMatch", "AI-IoT-Water-Quality-System", "Cafeteria-Ordering-App", "Ubuntu-Web-OS", "AI-Agent-Playground"],
            interests: ["Software Engineering", "Deep Learning", "Computer Vision", "Gen AI", "Full Stack Development"],
            languages: ["Java", "C++", "Java", "Dart", "JavaScript"],
        };
        this.state = {
            terminal: [],
        }
    ...
    reStartTerminal = () => {
        clearInterval(this.cursor);
        $('#terminal-body').empty();
        $('#terminal-body').append(this.ai.getGreeting());
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className=" text-ubt-green">{(this.props.user_name || "anmol").toLowerCase()}@Dell</div>
    ...
    handleCommands = (command, rowId) => {
        ...
        switch (main) {
            ...
            case "pwd":
                ...
                result = str.replace("~", "/home/anmol")
                break;
            case "code":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("vscode");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands:[ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-anmol, todoist, trash, settings, sendmsg]";
                }
                break;
            case "echo":
            ...
            case "about-anmol":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("about-anmol");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-anmol, todoist, trash, settings, sendmsg ]";
                }
                break;
            ...
            case "sudo":
                ...
                break;
            default:
                const aiResult = this.processAICommand(words.length ? main + " " + rest : main);
                if (aiResult) {
                    // Simulation of "thinking" - makes it feel like a real AI
                    document.getElementById(`row-result-${rowId}`).innerHTML = `
                        <div class="flex items-center animate-pulse text-gray-400">
                    ...
                    setTimeout(() => {
                        document.getElementById(`row-result-${rowId}`).innerHTML = aiResult;
                        this.appendTerminalRow();
                    }, 800);
                    return; // Return early to skip synchronous update
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-anmol, todoist, trash, settings, sendmsg ]<br><span class='text-ubt-blue'>Tip: Try asking me 'who is anmol', 'show ai projects', or 'why should we hire you'!</span>";
                }
        }
...
export const displayTerminal = (addFolder, openApp, user_name) => {
    return <Terminal addFolder={addFolder} openApp={openApp} user_name={user_name}> </Terminal>;
}
